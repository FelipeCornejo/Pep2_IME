...
#PASO 4
#Se har√° uso de la variable Wrists.diameter debido a multiples alusiones a sobre determinar obesidad
#en base a estas:
...
#Junto con la observaci√≥n de que gente muy flaca tiene las mu√±ecas huesudas y gente sobre peso las tiene hinchadas.
...
#Los residuos deben ser independientes entre s√≠.
...
p <- ggscatter( muestra , x = "Wrists.diameter", y = "Weight", color = "blue", fill = "blue",
                 xlab = "Diametro de la mu√±eca [cm]", ylab = "Peso [kg]")
=======
p <- ggscatter ( muestra , x = "Wrists.diameter", y = "Weight", color = "blue", fill = "blue",
                 xlab = "Diametro de la muÒeca [cm]", ylab = "Peso [kg]")
>>>>>>> Stashed changes
...
# SE puede apreciar que el grafico sigue una relaciÛn lineal entre ambas variables
# y el coeficiente de relaciÛn es 0,6791, lo cual en algunas de las lecturas se consideraria
# una relaciÛn fuerte. R > 0.5
...
#1. La distribuci√≥n de los residuos debe ser cercana a la normal.
cat("\ nPrueba de normalidad para los residuos :\n")
print( shapiro.test(modelo$residuals))
#2. La variabilidad de los residuos debe ser aproximadamente constante.
cat(" Prueba de homocedasticidad para los residuos :\n")
print(ncvTest( modelo ))
#3. Los residuos deben ser independientes entre s√.
cat("Prueba de Durbin - Watson para autocorrelaciones ")
cat("entre errores :\n")
...
cat(" Intervalo raz√≥n de covarianza : [", inferior , "; ",
       superior , "]\n\n", sep = "")
print(round( resultados[ sospechosos , c("distancia_Cook", "apalancamiento",
                                            "covratio")], 3))

#PASO 8
#Se implementa validaci√≥n cruzada.
n <- nrow(datos_7)
